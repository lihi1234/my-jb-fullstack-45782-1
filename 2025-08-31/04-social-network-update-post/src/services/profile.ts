...
class ProfileService {
    ...
    async editPost(id: string, draft: PostDraft): Promise<Post> {
       ...
       async newComment(draft: CommentDraft): Promise<PostComment> {
        const response = await axios.post<PostComment>(`${import.meta.env.VITE_REST_SERVER_URL}/comments`, draft)
        return response.data
    }
