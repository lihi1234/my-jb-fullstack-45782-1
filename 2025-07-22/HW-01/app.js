...
//all users
    console.log(users);
//full names array
    const usersFullNames= users.map(user=>{return{fullName: `${user.firstName} ${user.lastName}`}});
    console.log(usersFullNames);
//states array
const states= users.reduce((cumulative, user)=>{
    if(!cumulative.includes(user.company.address.state))
        cumulative.push(user.company.address.state);
    return cumulative;
},[])
console.log(states)

//eye color array
const eyeColorArr= users.reduce((cumulative, user)=>{
    const numOfEyeColor= users.filter(currentUser=> currentUser.eyeColor===user.eyeColor)

    if(cumulative.filter(current=> current.eyeColor===user.eyeColor).length===0)
    {
        cumulative.push({eyeColor:user.eyeColor, num:numOfEyeColor.length});
    }
...
//north array
const northUsers= users.filter(user=> user.address.coordinates.lat>0);

//north average
const northAverage= northUsers.reduce((cumulative, user)=>cumulative+user.weight,0)/northUsers.length
console.log(northAverage)

//south array
const southUsers= users.filter(user=> user.address.coordinates.lat<0);

//south average
const southAverage= southUsers.reduce((cumulative, user)=>cumulative+user.weight,0)/southUsers.length
console.log(southAverage)